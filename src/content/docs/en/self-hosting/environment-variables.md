...
```
# Variables marked [derived] are generated by `bin/setup --init`
# (which runs `bundle exec rake ots:secrets` internally) via HKDF from SECRET.
...
FEDERATION_SECRET=                    # Secret for federation between instances (auto-generated by `bin/setup --init` as a multi-word passphrase)
```

**Note**: "Colonel" is our term for "admin" users. Create a colonel directly with `bin/ots customers create email@example.com --role colonel` — this creates a verified account and prints a one-time generated password unless `--password` is given. To promote an existing account, use `bin/ots customers role promote email@example.com`. Colonels can access the admin area at `/colonel`. See [Run your own instance](/en/start/run-your-own-instance) for what the admin console covers.
...
CSP_ENABLED=true                      # Content Security Policy headers (default: true; only the literal value 'false' disables it)
...
# Available TTL options presented to users: space-separated seconds.
# Do NOT use commas — a comma-separated list is silently reduced to its first value.
