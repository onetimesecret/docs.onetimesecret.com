...
description: Regions are separate systems that never exchange data, so changing region means creating an account in the new one — this page covers how to do that, what happens to your old account, and how to move a custom domain without breaking outstanding links.
audience: end-user
pageType: how-to
---

Onetime Secret uses a [share-nothing architecture](/en/security/where-your-data-lives/) across all five regions (CA, EU, NZ, UK, US). Each region operates as a completely separate system with its own database, accounts, and secrets. We do not transfer data between regions under any circumstances.
...
Navigate directly to your preferred region (see [Available Regions](/en/security/where-your-data-lives/#available-regions) for the full list) and create a new account with the same email address. That's it — your new account is ready to use immediately.

## Paid Accounts (Identity Plus)

The process is the same as free accounts, with one extra step:

1. Go to your preferred region's URL (see [Available Regions](/en/security/where-your-data-lives/#available-regions))
