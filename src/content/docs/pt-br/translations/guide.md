---
title: Guia de estilo de tradução
...
Nosso público de língua portuguesa inclui:
...
- Público: Faixa etária diversa (18-80 anos), incluindo estudantes, idosos, pais, clientes, colegas de trabalho. Esses indivíduos podem nunca ter ouvido falar do Onetime Secret e geralmente têm menos contexto sobre como funciona.
...
*Orientação: Identifique regras de gramática, estilo e pontuação para manter a consistência em todo o seu conteúdo traduzido.*
...
- Em geral, use a segunda pessoa (`você`) para se dirigir ao usuário.
- Evite usar a primeira pessoa (`meu`, `eu`), pois pode criar confusão sobre quem está sendo abordado.

### Texto da Interface: Quando Usar Voz Ativa vs Passiva

#### Voz Ativa, Imperativa ("Faça algo")
...
Envio de formulário
✓ Salvar alterações (botão->ativo)
✓ Alterações salvas com sucesso (status->passivo)

Upload de arquivo
✓ Enviar arquivo (botão)
✓ Upload concluído (status)

## Consistência de terminologia

*Crie um glossário de termos-chave, juntamente com suas traduções aprovadas, para garantir o uso consistente em todo o conteúdo.*
...
Em "Onetime Secret", "Onetime" funciona como um adjetivo composto (também chamado de adjetivo atributivo) que modifica o substantivo "Secret". Descreve uma característica chave do segredo - que só pode ser acessado uma vez.
...
- **secret** (s.) - A informação confidencial sendo compartilhada
...
- **encryption** (s.) - O processo de codificação de segredos
...
#### Traduzindo a palavra "secret"

A palavra "secret" é central para nossa marca, produto e estratégia de comunicação. Protegemos segredos. Também fornecemos um serviço secreto.
...
CORRETO:   **Mensagem confidencial** ou **Conteúdo confidencial**
INCORRETO: Segredo (no contexto de informações pessoais)
...
✅ Você tem 3 novas mensagens confidenciais
❌ Você tem 3 novos segredos

### Traduzindo "password" vs "passphrase"

O Onetime Secret faz uma distinção importante entre dois conceitos de segurança que devem ser mantidos consistentemente em todas as traduções:

#### Password (Senha)
- **Definição**: Uma credencial de autenticação padrão usada para acesso à conta
...
- **Definição**: Uma medida de segurança especificamente para proteger segredos individuais
...
Ao traduzir esses termos:

1. Sempre mantenha a distinção consistente entre os dois conceitos
2. Teste seus termos escolhidos com falantes nativos não familiarizados com o produto
3. Certifique-se de que o termo passphrase transmita segurança/complexidade adicional em comparação com password
4. Use o mesmo termo para cada conceito em toda a tradução

✅ Abordagem correta:
Seção de conta: "Digite sua **senha** para fazer login"
Seção de segredo: "Este segredo está protegido com uma **frase secreta**"

❌ Abordagem incorreta:
Usar o mesmo termo para ambos os conceitos, ou alternar inconsistentemente entre termos

#### Gestão de Usuários

- **colonel** (s.) - Função de administrador
...
*Estabeleça padrões para formatação, layout e tipografia para manter a consistência visual e de design em diferentes idiomas.*
...
Geralmente, um relógio de 24 horas é usado no Brasil. Relógios de 12 horas são menos comuns.

### Numerais

|  Finalidade do Símbolo  |  Nome do Caractere  |  Símbolo  |  Exemplo  |
| --- | --- | --- | --- |
...
O sistema métrico é usado no Brasil. Principalmente m, km, cm, mm, kg, g.
...
Nosso objetivo: Queremos que nossa escrita se conecte efetivamente com todos os leitores, sendo clara, precisa e respeitosa.
...
- Use tradução literal para textos legais ou técnicos
