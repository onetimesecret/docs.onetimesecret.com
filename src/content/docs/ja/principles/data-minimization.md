...
**説明：** Onetime Secretでは、サービスを提供するために必要なデータのみを収集します：一時的なシークレットと、有料購読に必要な最低限のアカウント情報です。これにより、シンプルで安全なサービスを提供しています。
...
1.**単一のシークレットの場合：** 暗号化されたシークレットは、閲覧されるか有効期限が切れるまで保存されます。その後、それは永久に削除されます。(緊急事態に備えて保管する最小限の暗号化されたバックアップを除きます。これについては[プライバシーポリシー](https://onetimesecret.com/info/privacy)で詳しくしています)。
