...
|---------|-------------|-------------|-------------|-------------|-------|
| secret (noun, object/record) | Geheimnis | Geheimnis | secret | secret | The record being created, shared, viewed, burned, expired. See `_language-notes.md` §1. |
| secret (noun, revealed payload) | Nachricht | Nachricht | message | message | The decrypted body shown post-reveal, encrypted_message, truncated content. See `_language-notes.md` §1. |
...
- Der Begriff "secret" wird kontextabhängig übersetzt: `Geheimnis` für den Datensatz/das Objekt (erstellen, teilen, ansehen, verbrennen, ablaufen) und `Nachricht` für den entschlüsselten Inhalt nach dem Aufdecken (Post-Reveal-Anzeige, `encrypted_message`, gekürzte Inhalte). Diese Aufteilung gilt sowohl für `de` als auch für `de_AT`. Maßgebliche Referenz: `_language-notes.md` §1 sowie das de_AT-Baseline-Commit `f95b03f44`.
