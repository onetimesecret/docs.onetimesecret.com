...
带有自定义域名的一次性链接可为敏感数据的共享提供高度安全性，同时保持品牌的一致性和连续性。以下是客户使用我们服务的一些方式。

### 1.为客户入职或开发票

在要求客户提供敏感信息（如财务详情或信用卡信息）时，请客户通过在 secrets.example.com 上创建一次性链接来共享信息。这样可以确保信息安全共享，并且只有目标接收者才能访问。
