...
2. **Utiliser la protection par phrase secrète** : Pour les informations très sensibles, utilisez la fonction de protection par phrase secrète. Celle-ci ajoute un niveau de sécurité supplémentaire, en obligeant le destinataire à saisir une phrase secrète pour consulter le secret.
...
1. **Utiliser des environnements éphémères** : Dans la mesure du possible, créez et détruisez des environnements pour chaque session d'échange de secrets. Cela peut s'avérer particulièrement utile pour les opérations très sensibles.
