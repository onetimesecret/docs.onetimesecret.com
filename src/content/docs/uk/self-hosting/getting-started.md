---
title: Початок роботи
description: Швидкий посібник з налаштування для запуску вашого власного екземпляра Onetime Secret
sidebar:
  order: 2
---

Цей посібник допоможе вам за лічені хвилини запустити власний екземпляр Onetime Secret.

## Попередні вимоги

- **1ГБ+ оперативної пам'яті** для оптимальної продуктивності
- **Примітка щодо сховища Redis**: Залежно від конфігурації Redis, таємниці можуть зберігатися повністю в пам'яті без запису на диск для підвищення безпеки

## Метод 1: Docker (Рекомендовано)

Найшвидший спосіб почати роботу використовує Docker з мінімальною конфігурацією.

### 1. Запустіть Redis

```bash
docker run -p 6379:6379 -d redis:bookworm
```

### 2. Згенеруйте секретний ключ

```bash
# Згенеруйте та збережіть постійний секретний ключ
openssl rand -hex 32 > .ots_secret
chmod 600 .ots_secret
echo "Секретний ключ збережено в .ots_secret (зберігайте цей файл у безпеці!)"
```

### 3. Запустіть Onetime Secret

```bash
# Запустіть контейнер з секретним ключем
docker run -p 3000:3000 -d \
  -e REDIS_URL=redis://host.docker.internal:6379/0 \
  -e SECRET="$(cat .ots_secret)" \
  -e HOST=localhost:3000 \
  -e SSL=false \
  -e RACK_ENV=production \
  onetimesecret/onetimesecret:v0.26.4
```

### 4. Отримайте доступ до свого екземпляра

Відкрийте браузер за адресою:
- **Веб-інтерфейс**: http://localhost:3000
- **API-ендпоінт**: http://localhost:3000/api/v2/status

## Метод 2: Ручне встановлення

Для тих, хто віддає перевагу ручному налаштуванню, вам знадобиться:

- **Ruby 3.4+** (може бути недоступним у стандартних системних пакетах)
- **Redis 7+** або **Valkey** (альтернатива Redis)
- **Node.js 22+** та **pnpm** (потрібно лише для розробки та збирання фронтенд-ресурсів)

Вам потрібно буде зібрати фронтенд-ресурси за допомогою `pnpm install && pnpm run build:local` перед запуском додатку.

Перегляньте [README](https://github.com/onetimesecret/onetimesecret#readme) для повних деталей ручного встановлення.

## Перевірка

1. Перейдіть за адресою http://localhost:3000
2. Створіть тестову таємницю, щоб переконатися, що все працює
3. Перевірте статус API за адресою http://localhost:3000/api/v2/status

## Налаштування адміністратора

Коли Valkey/Redis запущено, а ваш `.env` завантажено в оболонку (`set -a; source .env; set +a`), створіть обліковий запис адміністратора безпосередньо:

```bash
bundle exec bin/ots customers create admin@example.com --role colonel
```

Це створює перевірений обліковий запис і виводить одноразово згенерований пароль — збережіть його — якщо ви не вкажете `--password`. Це працює як у простому, так і в повному режимі автентифікації. Щоб підвищити вже наявний обліковий запис:

```bash
bundle exec bin/ots customers role promote admin@example.com
```

**Примітка**: Зона адміністратора наразі має обмежений функціонал - це тільки перегляд конфігурації в режимі читання без управління користувачами. Більше функцій запланована для майбутніх випусків.

## Наступні кроки

Тепер, коли ваш екземпляр працює:

1. **[Налаштуйте ваше розгортання](./installation)** для виробничого використання
2. **[Перегляньте параметри конфігурації](./configuration)** для налаштування

## Отримання допомоги

- **Документація**: Перегляньте наш [довідник конфігурації](./configuration)
- **Спільнота**: Приєднуйтесь до обговорень на [GitHub](https://github.com/onetimesecret/onetimesecret)
- **Проблеми**: Повідомляйте про помилки в нашому [трекері проблем](https://github.com/onetimesecret/onetimesecret/issues)
