...
- **`config/config.yaml`** - Основна конфігурація додатку з використанням шаблонів ERB. Тут інтегровані змінні середовища, що дозволяє легко бачити, як застосовується кожне налаштування. (Скопіюйте з `etc/defaults/config.defaults.yaml`)
...
**Початок роботи:**
1. Скопіюйте приклад: `cp etc/defaults/config.defaults.yaml config/config.yaml`
...
:site:
  ...
  # проблем, таких як неможливість розшифрувати існуючі таємниці.
  ...
  # Параметри конфігурації для управління таємницями
  :secret_options:
    # Стандартний час життя (TTL) для таємниць у секундах
    # Це значення використовується, якщо конкретний TTL не вказано при створенні таємниці
    :default_ttl: <%= ENV['DEFAULT_TTL'] || nil %>
    # Доступні варіанти TTL для створення таємниць (у секундах)
    # Ці варіанти будуть представлені користувачам при створенні нової таємниці
    # Формат: Рядок цілих чисел, що представляють секунди
    :ttl_options: <%= (ENV['TTL_OPTIONS'] || nil) %>
    # Налаштування для поля пароля, яке захищає доступ до таємниць
    :passphrase:
      # Вимагати від користувачів вводити пароль при створенні таємниць
  ...
  :authentication:
    ...
    # Коли увімкнено, форма таємниці на головній сторінці недоступна, якщо
    ...
    # таємниці, зберігаючи при цьому навігацію та брендинг сайту.
    :required: <%= ENV['AUTH_REQUIRED'] == 'true' %>
    # Облікові записи Colonel (адміністратор) керуються через CLI:
...
:experimental:
  ...
  # - Таємниці можуть зберігатися без належного шифрування
  # - Можливий несанкціонований доступ до конфіденційних даних
  # - Цілісність таємного посилання не може бути гарантована
  #
  ...
  #    існуючі таємниці, створені в той час. Увімкніть тимчасово до тих пір, поки
  #    всі постраждалі таємниці не закінчаться (максимальний період TTL).
  ...
  #
  # ОБСЛУГОВУВАННЯ: Видаліть секрети з цього списку після того, як відповідні таємниці
