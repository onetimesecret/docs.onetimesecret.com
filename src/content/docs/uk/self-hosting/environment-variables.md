...
```bash
REDIS_URL=redis://localhost:6379/0   # Рядок підключення Redis для сесій, таємниць та всіх даних додатку
```
...
# Доступні варіанти TTL, що пропонуються користувачам: секунди, розділені пробілами.
# НЕ використовуйте коми — список, розділений комами, мовчки скорочується до першого значення.
