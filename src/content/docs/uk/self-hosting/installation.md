...
services:
  onetime:
    image: onetimesecret/onetimesecret:v0.26.4
    ports:
      - "3000:3000"
    environment:
...
# Безпека - таємниці ніколи не записуються на диск
...
**Варіант 2: Збереження на диск (дозволяє резервне копіювання, але записує таємниці на диск):**
...
**Важливо**: При увімкненому збереженні на диск таємниці будуть записуватися до:
