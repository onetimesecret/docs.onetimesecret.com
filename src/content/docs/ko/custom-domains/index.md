...
6. **지역적 유연성**: 데이터 위치 요구 사항을 충족하기 위해 다섯 개의 데이터 센터 리전(EU, UK, US, CA, NZ) 중에서 선택할 수 있습니다.
...
4. 4. 원하는 데이터 센터 지역(EU, UK, US, CA 또는 NZ)을 선택합니다.
5. DNS 구성을 위해 제공된 단계별 지침을 따릅니다.
6. 6. 도메인이 확인될 때까지 기다립니다. 가능한 경우 '확인' 버튼을 클릭하여 프로세스를 신속하게 진행합니다. 페이지를 새로 고쳐야 할 수도 있습니다.

선호하는 데이터 센터 지역을 선택하는 방법을 비롯한 자세한 지침은 [사용자 정의 도메인 설정 가이드](/ko/custom-domains/setup-guide)를 참조하세요.
...
자세한 브랜딩 지침은 [브랜드 가이드](/ko/custom-domains/brand-guide)를 참조하세요.
...
- [데이터 센터 지역](/ko/regions)을 선택할 때는 데이터 보호 규정을 고려하세요.
