...
올바른 선택: **비밀 메시지** (비밀 메시지)
주의할 선택: 비밀 (비밀)

"비밀 메시지"라는 표현은 비밀(예: 원타임 시크릿에서 생성된 비밀 메시지 또는 비밀 링크)을 언급할 때 사용해야 합니다. "비밀"이라는 단어만 사용하면 "비밀"의 직역이지만, 한국어 일상 사용에서는 개인적인 비밀이나 비밀스러운 정보의 의미가 강해 의도된 의미와 일치하지 않는 함의를 가지고 있습니다.

사용 예:

✅ 3개의 새 비밀 메시지가 있습니다 (You have 3 new secret messages)
❌ 3개의 새 비밀이 있습니다 (You have 3 new secrets)

이처럼 문맥에 따라 직역보다 의도한 기능을 정확히 전달하는 번역이 더 효과적입니다. UI 요소에서는 "비밀 메시지"를, 제품명이나 브랜드 맥락에서는 "원타임 시크릿"을 그대로 사용하는 것이 좋습니다.
...
- **secure** (보안)
- **private** (비공개)
- **password / passphrase** (비밀번호 / 접근 문구) - 아래 참조
- **expiration / expiry / time-to-live** (만료 / 만료 / 유효기간)

### "password"와 "passphrase" 번역하기

원타임 시크릿은 모든 번역에서 일관되게 유지되어야 하는 두 가지 보안 개념을 중요하게 구분합니다:

#### Password (비밀번호)
- **정의**: 계정 접근을 위한 표준 인증 자격 증명
- **사용 맥락**: 원타임 시크릿 플랫폼의 사용자 계정에 로그인할 때만 사용
- **예시**: "계정 비밀번호", "로그인 비밀번호", "비밀번호 재설정"
- **번역 지침**:
  - 한국어에서 인증 자격 증명에 대한 가장 일반적이고 표준적인 용어인 "비밀번호" 사용
  - 사용자가 웹사이트에 로그인할 때 기대하는 용어여야 함
  - 일반 사용자에게 더 친숙한 용어

#### Passphrase (접근 문구)
- **정의**: 개별 비밀 메시지를 보호하기 위한 보안 수단
- **사용 맥락**: 비밀 메시지를 생성하거나 접근할 때 사용 (계정 접근용이 아님)
- **예시**: "접근 문구로 보호", "접근 문구를 입력하여 보기", "접근 문구로 보호된 비밀"
- **번역 지침**:
  - 일반 계정 비밀번호와 명확하게 구분되어야 함
  - 잠재적으로 더 길고 구문 기반의 보호 메커니즘 개념을 전달해야 함
  - "접근 문구"는 이 특수한 보안 개념을 전달하는 일관된 복합 용어

#### 구분 유지하기

✅ 올바른 접근법:
- 계정 섹션: "로그인하려면 [비밀번호]를 입력하세요"
- 비밀 섹션: "이 비밀은 [접근 문구]로 보호됩니다"

❌ 잘못된 접근법:
- 두 개념에 동일한 용어를 사용하거나 용어를 일관성 없이 바꾸는 것
